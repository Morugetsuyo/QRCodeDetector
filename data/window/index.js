...
const processImageForQRCode = async (dataUrl, trial = 1) => {
  ...
  img.onload = async () => {
    ...
    try {
      await qrcode.ready();
      const detectionResults = qrcode.detectMultiple(canvas, canvas.width, canvas.height);

      if (detectionResults && detectionResults.length > 0) {
        const selectedQRCode = detectionResults[Math.floor(Math.random() * detectionResults.length)];
        highlightAndCenterQRCode(selectedQRCode);
        return;
      } else if (trial < 3) {
